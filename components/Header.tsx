...
const Header: React.FC<HeaderProps> = ({
  ...
  const logoHoverTextColor = applyScrolledStyle
    ? "hover:text-deep-green"
    : "hover:text-rose-gold";
  const navLinkTextColor = applyScrolledStyle
    ? "text-charcoal-gray"
    : "text-white";
  ...
  return (
    <header className={headerClasses}>
      <div
        className={`container mx-auto max-w-6xl flex justify-around md:justify-between items-center px-4 md:px-6
          ...
          <ul className="flex justify-between md:space-x-8 w-full">
            <li className="px-2">
              <a
                href="#bride-groom"
                onClick={scrollToSection("bride-groom")}
                className={`flex flex-col items-center ${navLinkTextColor} ${navLinkHoverTextColor} font-medium transition-colors`}
              >
