...
const GalleryImageItem: React.FC<GalleryImageItemProps> = ({ src, alt }) => {
  ...
  useEffect(() => {
    ...
    setError(false);    // Reset error state on src change
    const img = new Image();
    img.src = src;
    img.onload = () => {
      setIsLoaded(true);
    ...
    <div className="aspect-w-4 aspect-h-3 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 group">
      ...
      {isLoaded && !error && (
        <img
          src={src}
          alt={alt}
          className="object-cover w-full h-full transition-opacity duration-500 opacity-0 group-hover:opacity-100"
          style={isLoaded ? { opacity: 1 } : {}} // Apply opacity once loaded for smooth transition
          onLoad={(e) => (e.currentTarget.style.opacity = '1')} // Ensure opacity is set explicitly on load
        />
      )}
    </div>
  );
